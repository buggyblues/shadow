...
for (const file of files) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      line.match(/<Pressable\b/) &&
      line.match(/style=\{styles\.(\w+)\}/) &&
      !line.includes('({pressed})')
    ) {
    ...
    if (line.match(/<Pressable\s+onPress=/) && line.includes('>') && !line.includes('style=')) {
      const newLine = line
        .replace(/>$/, ' style={({pressed}) => ({opacity: pressed ? 0.7 : 1})}>')
        .replace(/>\s*$/, ' style={({pressed}) => ({opacity: pressed ? 0.7 : 1})}>')
      result.push(newLine)
      continue
    }
