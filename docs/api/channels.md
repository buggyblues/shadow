...
- `lastMessagePreview`: optional preview of the latest top-level message, shaped as `{ id, content, createdAt, attachmentCount, author }`. `author` is `null` when the author is unavailable.
