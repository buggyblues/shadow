...
Channel chat routes should use the bootstrap endpoint for first paint instead of issuing separate access, channel, message, member, and slash-command requests.
...
- Message, member, and slash-command data are only included when the caller can read the channel.
