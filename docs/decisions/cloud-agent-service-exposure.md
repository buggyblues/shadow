# Shadow Cloud Agent Service Exposure 与 Space App 挂载方案

> **Status:** Proposed
> **Date:** 2026-06-23
> **Scope:** `apps/cloud`, `apps/server`, Shadow Cloud SaaS, agent runtime
> **Goal:** 将 agent 容器内启动的 HTTP 服务安全挂载到 `shadowob.com` 子域名，并支持 agent 启动符合 Space App 标准的应用后，通过 `shadowob-cli` 安装到指定 server。
...
更重要的目标是打通 agent 容器和 Shadow server 内的应用系统：agent 可以在容器里生成或启动一个标准 Space App，提供 `/.well-known/space-app.json`、`/shadow/server`、Space App-owned `/api/*`、`/.shadow/commands/:commandName` 等标准入口，然后把它安装到某个 Shadow server。用户和 Buddy 看到的是正常的 Space App；Cloud exposure 只是给这个 App 后端提供受控 HTTPS 入口。
...
本方案要求保持这些安全属性：**公网入口只能经过 Shadow 管控的 gateway，agent 容器不能直接持有公网 tunnel 凭据，也不能任意指定 upstream URL；安装 Space App 也必须经过 Shadow Server 的 Actor / PolicyService 授权，不能仅凭容器内写文件或持有普通用户 token 完成。**

## 目标

1. 支持模板中静态声明要暴露的服务端口。
2. 支持容器运行时动态声明要暴露的服务端口。
3. 支持 agent 容器内的标准 Space App 通过 CLI 暴露 manifest URL，并安装到目标 server。
...
Space App 挂载在这条 exposure 链路之上：

```text
agent container starts Space App on localhost:<port>
  -> shadowob space-app expose/publish validates policy
  -> exposure gateway serves/reaches public manifest URL
  -> shadowob space-app preview/install uses that manifest URL
...
| `shadowob space-app` CLI | agent 容器内的主要操作面；把 expose、manifest 发现、Space App install、默认权限和 Buddy grant 串成可审计流程。 |

## Agent-hosted Space App 目标形态

Agent 启动的应用必须是普通 Space App，而不是 Cloud 专用协议。最低要求沿用现有标准：

- `/.well-known/space-app.json`：`shadow.space-app/1` manifest。
- `/shadow/server`：server 内嵌页面入口。
- `/assets/...`：静态资源。
- `/api/*`：Space App-owned 业务 API，只服务 Space App UI 和 Space App session。
- `/.shadow/commands/:commandName`：Shadow gateway command ingress，只接受 Shadow 转发。
- 后端使用 SDK helper 处理 manifest、Shadow command token、command schema 和 actor context。
- 前端只调用 Space App-owned `/api/*`，iframe bridge 只做宿主体验增强。
...
| Space App mount layer | 读取 manifest，把 App 安装/更新到某个 server，并设置默认权限或 Buddy grant。 | `server:{id}`、`space_app_installation:{serverId}:{appKey}` |

推荐 CLI 提供一个组合命令，内部复用现有 `shadowob space-app preview/install/defaults/grant` 能力：

```sh
shadowob space-app publish \
  --deployment "$SHADOWOB_CLOUD_DEPLOYMENT_ID" \
  --agent "$SHADOWOB_AGENT_ID" \
  --server <server-id-or-slug> \
  --app-key kanban \
  --port 4201 \
  --manifest-file ./space-app.local.json \
  --visibility private \
...
```sh
publish_json="$(shadowob space-app expose \
  --deployment "$SHADOWOB_CLOUD_DEPLOYMENT_ID" \
  --agent "$SHADOWOB_AGENT_ID" \
  --app-key kanban \
  --port 4201 \
  --manifest-path /.well-known/space-app.json \
  --visibility private \
  --json)"

manifest_url="$(printf '%s' "$publish_json" | jq -r '.manifestUrl')"

shadowob space-app preview \
  --server <server-id-or-slug> \
...
shadowob space-app defaults kanban \
  --server <server-id-or-slug> \
  --permissions cards.read,cards.create \
  --json
```
...
```sh
shadowob space-app grant kanban \
  --server <server-id-or-slug> \
  --buddy <buddy-id> \
...
第一版可以先实现拆分命令；`publish` 作为 CLI 便利封装。这样 Cloud exposure 与 Space App 安装可以独立测试和审计。
...
- `allowedCliCommands`，例如 `space-app.expose`、`space-app.publish`、受限 `space-app.preview`、受限 `space-app.install`。
- 过期时间和可撤销 token id。

`shadowob space-app install/defaults/grant` 在普通用户机器上仍可使用完整用户授权；在 agent 容器里调用时，server 必须识别 agent actor，并用 PolicyService 校验它是否被允许管理目标 server 的指定 appKey。OAuth/PAT scope 或 CLI command 白名单不能替代资源授权。

### Manifest URL 与重写

Space App 可以在容器内只知道本地监听端口，不需要提前知道公网域名。推荐控制面返回：

```json
{
  "exposureId": "exp_...",
  "url": "https://e-01jz-kanban.run.shadowob.com",
  "manifestUrl": "https://e-01jz-kanban.run.shadowob.com/.well-known/space-app.json"
}
```
...
2. `shadowob space-app expose` 可以只暴露原始 manifest；`shadowob space-app publish` 可以要求控制面先通过 gateway discover，再安装。
...
上一版方案解决了“如何安全访问容器内服务”和“如何通过 CLI 安装 Space App”，但长期可用性还缺三件事：
...
3. **状态备份**：Space App 的业务数据不能散落在随机路径。必须有标准 state dir / volume contract，并纳入 Cloud 控制台现有 backup/restore 链路。
...
| `installed` | 已安装到 server 的 Space App。 | 稳定 host，不随 pod/exposure 变化。 | 必须绑定 code release 和 state policy。 |
| `promoted` | 用户确认长期运行的 App。 | 稳定 host + 自动恢复。 | 强制 scheduled backup、restore 演练、明确保留策略。 |

`shadowob space-app publish` 默认创建 `installed` release；普通 `shadowob space-app expose` 仍只创建 `preview` exposure。需要只创建 release 而不安装到 server 时，显式传 `--no-install`。

### 稳定 Host

Space App 安装时不应保存 `https://e-01jz-kanban.run.shadowob.com/...` 这种一次性 exposure host。应保存稳定 host：

```text
https://app-<installation-id>.run.shadowob.com/.well-known/space-app.json
```

或：

```text
https://<app-key>-<deployment-id>.apps.run.shadowob.com/.well-known/space-app.json
```

稳定 host 指向 `cloud_app_releases.current_exposure_id`。Pod 重启、Service 重建、runtime exposure host 变化时，只更新 registry 映射，不需要重新安装 Space App。若 runtime 暂时不可用，gateway 返回可诊断的 `503 app starting/degraded` 页面，而不是让 manifest URL 静默变成 404。
...
1. **一个可恢复单元**：用户看到的是一个 Cloud App，所以备份也必须围绕 Cloud App，而不是只围绕 PVC、只围绕源码或只围绕 Space App installation。
...
Agent-hosted Space App 必须把代码、构建产物和业务状态分层：

```text
/workspace/space-apps/<appKey>/source      # 源码，可由 agent 生成或从 Git clone
/workspace/space-apps/<appKey>/dist        # 构建产物，可重新生成
/state/space-apps/<appKey>/data            # 业务状态，必须纳入 state component
/state/space-apps/<appKey>/uploads         # App 私有上传文件，必须纳入 state component
...
SHADOWOB_APP_SOURCE_DIR=/workspace/space-apps/kanban/source
SHADOWOB_APP_STATE_DIR=/state/space-apps/kanban/data
SHADOWOB_APP_RELEASE_DIR=/workspace/space-apps/kanban/dist
SHADOWOB_APP_PUBLIC_BASE_URL=https://app-sai-....run.shadowob.com
SHADOWOB_APP_API_BASE_URL=https://app-sai-....run.shadowob.com
```

Space App manifest 或 publish request 需要声明持久状态：

```json
{
  "backup": {
    "state": {
      "paths": ["/state/space-apps/kanban/data", "/state/space-apps/kanban/uploads"],
      "externalStores": []
    },
...
建议 Space App runtime 支持内部 hook，不通过公网暴露：

```json
{
  "backup": {
    "hooks": {
      "preparePath": "/.shadow/backup/prepare",
      "commitPath": "/.shadow/backup/commit",
      "restorePreparePath": "/.shadow/restore/prepare",
      "restoreVerifyPath": "/.shadow/restore/verify"
    }
  }
}
```
...
- **`shadowob-cli`**：显式操作入口。Agent 主容器使用短期 agent CLI profile 调用 `shadowob space-app expose/publish/status/unpublish`，适合 Space App 发布、安装和授权编排。
...
如果动态声明的是 Space App，它只表达“这个端口上运行了标准 Space App”，不等价于安装到 server：
  ...
  "exposures": [
    {
      ...
      "spaceApp": {
        ...
        "apiBasePath": "/",
        "sourcePath": "/workspace/space-apps/kanban/source",
        "statePaths": ["/state/space-apps/kanban/data"]
      }
    }
  ]
}
...
sidecar 可以把它暴露成 manifest URL，并写入 `status.json`。安装到哪个 server、授予哪些权限，仍建议由 `shadowob space-app publish --server ...` 或显式的 `shadowob space-app install/defaults/grant` 完成。
...
interface ExposureRequest {
   ...
   */
  id: string
  /** 普通 HTTP 服务或标准 Space App。默认 http_service。 */
  ...
  pathPrefix?: string
  /** kind=space_app 时必填。 */
  spaceApp?: SpaceAppExposure
  tags?: string[]
}

interface SpaceAppExposure {
  /** 必须与 manifest.appKey 一致。 */
  appKey: string
  /** 容器内 manifest path，默认 /.well-known/space-app.json。 */
  manifestPath?: string
  ...
  "exposures": [
    ...
    },
    {
      ...
      "spaceApp": {
        ...
        "stableUrl": null,
        "installed": false,
        "installReason": "run shadowob space-app publish --server <server>"
      },
      "reason": null,
...
```json
{
  "dynamicExposePolicy": {
    "enabled": true,
    "allowedKinds": ["http_service", "space_app"],
    ...
    "allowedSpaceAppKeys": ["kanban", "skills-*"],
    "allowedServerIds": ["srv_..."],
    "allowedStateRoots": ["/state/space-apps"],
    "maxStatePaths": 8,
    "maxTtlSeconds": 86400,
...
6. `kind=space_app` 时，`spaceApp.appKey` 必须匹配 `allowedSpaceAppKeys`，并与实际 manifest 的 `appKey` 一致。
7. 文件里的 `requestedServerId` 只作为提示；`allowFileRequestedInstall=false` 时不得触发安装。即使开启，也必须再检查 agent actor 对目标 server 的 `manage space_app_installation` 权限。
8. `spaceApp.statePaths[]` 必须落在 `allowedStateRoots` 内；路径只能作为备份声明，不能让容器选择任意 host path/PVC。
...
`shadowob space-app publish` 可以调用一个更高层 API，把 exposure 和 Space App mount 作为一个事务编排：
...
8. 调用现有 Space App install service，使用同一个 agent actor 触发 `manage space_app_installation` policy。
...
cloud_exposures
  ...
  exposure_kind       -- http_service | space_app
  release_id
  host
  stable_host
  protocol
...
Space App exposure 的特殊路由：

- `GET /.well-known/space-app.json` 可以返回 upstream manifest，或返回 gateway synthesized manifest。
- `GET /shadow/server` 是 iframe entry；Shadow host 通过受控 `postMessage` 在内存中下发 launch context，不在 iframe URL 中追加凭证。
- `/api/*` 是 Space App-owned 业务 API，gateway 只做普通反向代理和 exposure policy。
- `/.shadow/commands/:commandName` 只接受 Shadow gateway command 调用，不接受浏览器伪造调用。
...
- actor kind：`user | pat | oauth | service_token | space_app_launch | space_app_command | agent | anonymous`
...
| visibility | 行为 |
|------------|------|
| `private` | 普通 preview 必须登录 Shadow，PolicyService 校验 deployment/exposure read 权限；Space App iframe 使用 Space App session 或 Shadow OAuth 关联；Space App command 只通过 Shadow gateway token。 |
| `signed` | 允许短期签名链接；默认只允许 `GET/HEAD`；写方法仍要求 `private`。 |
| `public` | 允许匿名访问，但必须经过 WAF、限流、审计和模板/用户显式批准。 |

Space App 相关授权：

- Space App-owned `/api/*` 不接收 Shadow 主站用户 JWT、PAT 或 command token；Space App backend 使用自己的 session/OAuth 边界。
- `/.shadow/commands/...` 可以转发 Shadow gateway command token；gateway 必须确认请求来自 Shadow server 服务身份或可信 mTLS/service token，不能让公网浏览器直接带任意 bearer token 调 command。
- 安装、默认权限、Buddy grant 属于 `manage space_app_installation`，不属于 `cloud_exposure.read`，必须在 Space App service 中再次校验。
...
- 只在 `/.shadow/*` allowlist 路径转发 Shadow gateway token；令牌必须有可识别 audience/type，且不能是普通用户 JWT/PAT。
...
- 对 Space App 子域，只允许 host-only、SameSite、Secure cookie；不得设置 `Domain=.shadowob.com`。
...
- 对 `kind=space_app` 的 `/shadow/server`，根据 manifest `iframe.allowedOrigins` 生成 CSP `frame-ancestors`，只允许 Shadow host。
...
- Space Apps：显示 `appKey`、stable manifest URL、安装 server、release version、安装状态、默认权限和 Buddy grants。
- Denied：显示拒绝原因，例如 `port outside allowed range`。
- Actions：copy URL、extend TTL、make private、revoke。

动态 preview 服务的 URL 不保证永久稳定。已安装 Space App 必须显示 stable host；默认重启后由 controller 恢复 stable host 到 active runtime 的映射。
...
Agent 生成 Space App 前必须能读到标准开发资料。这些资料通过标准 Skill 包注入 runtime：

- Cloud runtime 由 `shadowob` plugin 在 `runtimeExtensions.shadowob.officialSkills` 中声明
  `shadowob` 和 `shadow-space-app`。
...
Agent 在容器里完成标准 Space App 发布的推荐脚本：
...
shadowob space-app publish \
  ...
  --port 4201 \
  --manifest-file ./space-app.local.json \
  --source-path /workspace/space-apps/"$SHADOWOB_APP_KEY"/source \
  --state-paths /state/space-apps/"$SHADOWOB_APP_KEY"/data \
  --visibility private \
  --json
```
...
```json
{
  "release": {
    ...
    "manifestUrl": "https://app-sai-....run.shadowob.com/.well-known/space-app.json",
    "codeSha256": "sha256:..."
  },
  "exposure": {
    "id": "exp_...",
...
- `shadowob space-app expose`：只创建/更新 exposure，返回 URL/manifestUrl。
- `shadowob space-app publish`：创建 code release、stable host、exposure，可选 install/defaults/grants/backup。
- `shadowob space-app status --app-key <key>`：查询 exposure 与安装状态。
- `shadowob space-app backup --app-key <key>`：立即备份 code/state/metadata。
- `shadowob space-app restore --app-key <key> --backup <id>`：恢复 code/state/metadata，可选择恢复到新 appKey。
- `shadowob space-app unpublish --app-key <key>`：关闭 exposure，可选卸载 Space App 或保留安装但标记 unavailable。
- `shadowob space-app install/defaults/grant/uninstall`：继续作为通用 Space App 管理命令，不只服务 Cloud。
...
| Agent 运行 `shadowob space-app expose` | 创建或更新 `kind=space_app` exposure，返回 public base URL 和 manifest URL。 |
| Agent 运行 `shadowob space-app publish` | 创建 code release 和 stable host；expose 后 discover manifest；安装/更新 Space App；设置默认权限和 Buddy grant；按策略备份。 |
...
| Agent 运行 `shadowob space-app unpublish` | 关闭 exposure；根据参数保留或卸载 Space App 安装；写审计记录。 |
...
| Deployment resume | controller 恢复 installed/promoted Space App runtime；静态 exposure 恢复；preview 等 runtime 重新声明。 |
...
| Agent 试图安装到未授权 server | `shadowob-cli` 使用 agent actor；Space App install/defaults/grant 必须 PolicyService 校验 server 资源权限和 appKey allowlist。 |
| Agent 用动态 JSON 越权安装 App | desired-state 文件默认只创建 exposure；`allowFileRequestedInstall=false`；安装必须走 CLI/API 授权。 |
| 一次性 exposure 过期导致已安装 App 失效 | Space App 安装使用 stable host；controller 持续维护 stable host 到 active runtime 的映射。 |
...
| 浏览器伪造 Space App command | `/.shadow/commands/*` 只接受 Shadow server gateway 身份和短期 command token；Space App backend 仍按 SDK 校验。 |
| Manifest 指向内网或 localhost | discovery 使用 SSRF guard，拒绝 private/link-local/cluster DNS/localhost 和危险 redirect。 |
| Manifest appKey 漂移 | `spaceApp.appKey`、CLI `--app-key`、manifest `appKey` 必须一致。 |
...
- 给显式启用动态 expose 且配置了专用 sidecar 镜像的 agent pod 注入 `emptyDir`：
  - `/run/shadow/exposure`
- 注入 `shadow-exposure-agent` sidecar；不复用 runner 镜像，避免 runner CLI 版本不包含 `shadowob space-app watch-exposures` 时拖垮主 Pod readiness。
- sidecar 读取 `desired.json`，调用 reconcile API。
- 支持 `private` 和短期 `signed`。
- UI 展示 runtime accepted/denied 状态。

### Phase 2.5：Agent-hosted Space App publish

- 新增 `kind=space_app` exposure。
- 新增 agent CLI profile/token，actor kind 为 `agent`。
- 新增 `shadowob space-app expose/status/unpublish`。
- 新增 `cloud_app_releases` 和 `cloud_app_backups`。
- 新增 `shadowob space-app publish`，组合 code release、stable host、exposure、manifest discover、install、defaults、grant。
- Space App discovery 增加 Shadow exposure manifest allowlist 和 SSRF guard。
- Gateway 支持 synthesized manifest 和 Shadow gateway command token 转发规则。
...
| `apps/server/src/handlers/cloud-exposure.handler.ts` | 实现 runtime reconcile、user revoke、signed link、Space App publish API。 |
...
| `apps/server/src/services/space-app.service.ts` | 复用现有 discover/install/defaults/grant；增加 agent actor 资源授权和 exposure manifest allowlist。 |
| `apps/server/src/services/policy.service.ts` | 增加 `cloud_exposure:read/manage`、`space_app_installation:manage` 的 agent actor 授权。 |
| `packages/cli/src/commands/space-app.ts` | 增加 `space-app expose/publish/status/backup/restore/unpublish`，并复用现有 Space App command 语义。 |
| `packages/sdk/src/client.ts` | 增加 Cloud exposure 与 Space App publish API client。 |
...
- state paths 默认限制在 `/state/space-apps/<appKey>`。
...
- `shadowob space-app publish` 必须创建 code release artifact 和 stable host。
...
4. `shadowob space-app publish` 用于 agent 容器内发布标准 Space App，并复用现有 `shadowob space-app install/defaults/grant` 的安装与授权模型。
5. 已安装 Space App 使用 `cloud_app_releases` stable host，而不是一次性 exposure host。
