...
更重要的目标是打通 agent 容器和 Shadow server 内的应用系统：agent 可以在容器里生成或启动一个标准 Server App，提供 `/.well-known/shadow-app.json`、`/shadow/server`、App-owned `/api/*`、`/.shadow/commands/:commandName` 等标准入口，然后把它安装到某个 Shadow server。用户和 Buddy 看到的是正常的 Server App；Cloud exposure 只是给这个 App 后端提供受控 HTTPS 入口。
...
- `/assets/...`：静态资源。
- `/api/*`：App-owned 业务 API，只服务 App UI 和 App session。
...
- 前端只调用 App-owned `/api/*`，iframe bridge 只做宿主体验增强。
...
- `/api/*` 是 App-owned 业务 API，gateway 只做普通反向代理和 exposure policy。
...
| visibility | 行为 |
|------------|------|
| `private` | 普通 preview 必须登录 Shadow，PolicyService 校验 deployment/exposure read 权限；Server App iframe 使用 App session 或 Shadow OAuth 关联；Server App command 只通过 Shadow gateway token。 |
| `signed` | 允许短期签名链接；默认只允许 `GET/HEAD`；写方法仍要求 `private`。 |
| `public` | 允许匿名访问，但必须经过 WAF、限流、审计和模板/用户显式批准。 |

Server App 相关授权：

- App-owned `/api/*` 不接收 Shadow 主站用户 JWT、PAT 或 command token；App backend 使用自己的 session/OAuth 边界。
...
| 浏览器伪造 Server App command | `/.shadow/commands/*` 只接受 Shadow server gateway 身份和短期 command token；App backend 仍按 SDK 校验。 |
