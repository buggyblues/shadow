...
agent container starts Server App on localhost:<port>
  -> shadowob cloud app expose/publish validates policy
...
| `shadowob cloud app` CLI | agent 容器内的主要操作面；把 expose、manifest 发现、server app install、默认权限和 Buddy grant 串成可审计流程。 |
...
shadowob cloud app publish \
  --deployment "$SHADOW_CLOUD_DEPLOYMENT_ID" \
  --agent "$SHADOW_CLOUD_AGENT_ID" \
  ...
  --visibility private \
  --install \
  --default-permissions cards.read,cards.create \
  --json
```
...
```sh
publish_json="$(shadowob cloud app expose \
  --deployment "$SHADOW_CLOUD_DEPLOYMENT_ID" \
  --agent "$SHADOW_CLOUD_AGENT_ID" \
  --app-key kanban \
  --port 4201 \
...
```env
SHADOW_CLI_PROFILE=cloud-agent
SHADOW_CLOUD_DEPLOYMENT_ID=dep_...
SHADOW_CLOUD_AGENT_ID=builder
```
...
1. Agent 生成的 manifest 可以使用相对路径，或通过 `SHADOW_APP_PUBLIC_BASE_URL` / `SHADOW_APP_API_BASE_URL` 读取 gateway 注入的公开 base URL。
2. `shadowob cloud app expose` 可以只暴露原始 manifest；`shadowob cloud app publish` 可以要求控制面先通过 gateway discover，再安装。
...
`shadowob cloud app publish --install` 默认创建 `installed` release；普通 `cloud app expose` 仍只创建 `preview` exposure。
...
| App 数据写在临时目录 | 重启后业务状态丢失。 | 强制使用 `SHADOW_APP_STATE_DIR` 或声明 `state.paths[]`，并挂载到 persistent state PVC。 |
...
```env
SHADOW_APP_KEY=kanban
SHADOW_APP_SOURCE_DIR=/workspace/server-apps/kanban/source
SHADOW_APP_STATE_DIR=/state/server-apps/kanban/data
SHADOW_APP_RELEASE_DIR=/workspace/server-apps/kanban/dist
SHADOW_APP_PUBLIC_BASE_URL=https://app-sai-....run.shadowob.com
SHADOW_APP_API_BASE_URL=https://app-sai-....run.shadowob.com
```
...
- durable data 只能写入 `SHADOW_APP_STATE_DIR` 或 policy 允许的 `state.paths[]`。
...
- **`shadowob-cli`**：显式操作入口。Agent 主容器使用短期 agent CLI profile 调用 `shadowob cloud app expose/publish/status/unpublish`，适合 Server App 发布、安装和授权编排。
...
```sh
cat > "$SHADOW_EXPOSURE_CONFIG.tmp" <<'JSON'
{
  "version": 1,
  "exposures": [
    {
...
JSON
mv "$SHADOW_EXPOSURE_CONFIG.tmp" "$SHADOW_EXPOSURE_CONFIG"
```
...
sidecar 可以把它暴露成 manifest URL，并写入 `status.json`。安装到哪个 server、授予哪些权限，仍建议由 `shadowob cloud app publish --server ... --install ...` 或显式的 `shadowob app install/defaults/grant` 完成。
  ...
  "exposures": [
    ...
    },
    {
      ...
      "serverApp": {
        ...
        "stableUrl": null,
        "installed": false,
        "installReason": "run shadowob cloud app publish --server <server> --install"
      },
      "reason": null,
...
`shadowob cloud app publish` 可以调用一个更高层 API，把 exposure 和 Server App mount 作为一个事务编排：
...
shadowob cloud app publish \
  --deployment "$SHADOW_CLOUD_DEPLOYMENT_ID" \
  --agent "$SHADOW_CLOUD_AGENT_ID" \
  --server "$SHADOW_TARGET_SERVER" \
  --app-key "$SHADOW_APP_KEY" \
  --port 4201 \
  --manifest-path /.well-known/shadow-app.json \
  --source-path /workspace/server-apps/"$SHADOW_APP_KEY"/source \
  --state-path /state/server-apps/"$SHADOW_APP_KEY"/data \
  --visibility private \
  --install \
  --backup-on-publish \
  --json
...
- `shadowob cloud app expose`：只创建/更新 exposure，返回 URL/manifestUrl。
- `shadowob cloud app publish`：创建 code release、stable host、exposure，可选 install/defaults/grants/backup。
- `shadowob cloud app status --app-key <key>`：查询 exposure 与安装状态。
- `shadowob cloud app backup --app-key <key>`：立即备份 code/state/metadata。
- `shadowob cloud app restore --app-key <key> --backup <id>`：恢复 code/state/metadata，可选择恢复到新 appKey。
- `shadowob cloud app unpublish --app-key <key>`：关闭 exposure，可选卸载 server app 或保留安装但标记 unavailable。
...
| Agent 运行 `shadowob cloud app expose` | 创建或更新 `kind=server_app` exposure，返回 public base URL 和 manifest URL。 |
| Agent 运行 `shadowob cloud app publish --install` | 创建 code release 和 stable host；expose 后 discover manifest；安装/更新 server app；设置默认权限和 Buddy grant；按策略备份。 |
...
| Agent 运行 `shadowob cloud app unpublish` | 关闭 exposure；根据参数保留或卸载 Server App 安装；写审计记录。 |
...
| App 状态写入临时目录 | 标准化 `SHADOW_APP_STATE_DIR`；promoted App 必须声明 state paths 并纳入 backup policy。 |
...
- 新增 `shadowob cloud app expose/status/unpublish`。
- 新增 `cloud_app_releases` 和 `cloud_app_backups`。
- 新增 `shadowob cloud app publish`，组合 code release、stable host、exposure、manifest discover、install、defaults、grant。
...
| `packages/cli/src/commands/cloud.ts` | 增加 `cloud app expose/publish/status/backup/restore/unpublish`，并复用现有 app command 语义。 |
...
- `shadowob cloud app publish --install` 必须创建 code release artifact 和 stable host。
...
4. `shadowob cloud app publish` 用于 agent 容器内发布标准 Server App，并复用现有 `shadowob app install/defaults/grant` 的安装与授权模型。
