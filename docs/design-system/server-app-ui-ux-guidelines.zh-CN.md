# Server App UI/UX 设计规范

> Draft v0.3. 采用平台设计规范的写法：先给原则，再给具体结构、尺寸、组件行为和验收方法。规范不点名具体 App，但提供可直接套用的页面模板。

## 设计原则

### 让用户知道自己在哪

每个 Server App 的首屏都要同时回答：
...
工具型 App 优先显示列表、表格、详情和操作状态。内容型 App 优先显示搜索、筛选、正文和作者信息。画布或实时 App 可以全屏，但必须保留模式、工具、状态和退出路径。
...
宿主 WebView 负责浏览器级操作：关闭、刷新、外部打开、宿主返回。App 负责自己的路由和内容状态。

Do:

- App 内 back 只返回 App 内上一层。
- 关闭 App 使用宿主 chrome。
- 避免左侧边缘滑动直接退出 WebView。

Don’t:

- 在 App 页面里重复做浏览器前进、后退、关闭按钮。
- 让 App 内返回和宿主返回竞争。
...
3. 渲染 App shell skeleton。
4. 加载首屏数据。
5. 数据返回后显示可操作界面。

Required behavior:

- 如果 launch context 缺失，显示“无法打开 App”错误页，并提供重新加载。
...
3. 如果 App router 有上一层，返回上一层。
4. 如果已经在 App 根路由，交给宿主关闭或保持当前页。
...
- 提交后 query 写入 URL 或 App route state。
- 清除按钮出现于非空输入状态。
...
- 底部 Tabbar 用于主要目的地。
- 底部 Tabbar 切换不应重建全局 App context。
...
- App back does not close the WebView.
- Host chrome does not cover App controls.
...
- Principle: applies to most Server Apps.
...
Keep examples generic. Document concrete UI structures, not the names of current Apps.
