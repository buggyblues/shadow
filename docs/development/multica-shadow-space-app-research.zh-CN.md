# Multica 能力复刻：Buddy Inbox + Space App 方案
...
Shadow 不需要在核心里内置 Kanban、Issue、Task Queue 等业务模型。用现有 **channel + message cards + Space App command** 可以覆盖 Multica 的用户可见能力：
...
- **领域对象属于 Space App**：Kanban card、GitHub issue、Linear issue、CRM ticket 等都由 Space App 自己实现。
- **Skills 是独立 Space App**：技能库不放进 Kanban；`skills` 负责发布、读取、导出和安装记录。
...
复刻 Multica 的关键不是复制表结构，而是复刻这条链路：业务事件 -> Space App outbox -> Buddy Inbox Task Card -> Buddy runtime claim -> 执行 -> 回写 card status/progress。
...
- Space App command 统一通过 `shadow.protocol === "shadow.space-app/1"` + `shadow.outbox.inboxTasks` 声明 Inbox 投递，Shadow 自动投递到 Buddy Inbox。
...
- Kanban 只保留通用 board/card/issue 状态能力；任务投递通过 Space App Backend -> Shadow REST 或 Space App outbox 写入 Buddy Inbox，由协调 Buddy 负责拆分、派发和验收。
- Skills 已拆成独立 `skills` Space App，并改为 Vite + React + TanStack Query 前端。
- Skills Space App 按完整 skill package 存储，不再把技能当单文件：`SKILL.md` 是入口，`references/`、`scripts/`、`assets/`、`examples/` 是一等文件。
- Kanban 中的 Autopilot 与技能库命令已删除。
- Trainer 已改成 Vite + TanStack Router 的多页面 Space App：`/problems`、`/submissions`、`/import`、`/problems/$challengeId`、`/problems/$challengeId/submissions/$submissionId`。
- Trainer 已验证 “提交代码 -> Strategy Buddy Inbox task -> Buddy 调 Space App command 获取提交 -> Buddy 写回分析 -> 前端实时显示结果”。
- Skills Space App 当前命令收敛为 `skills.list`、`skills.get`、`skills.download`、`skills.upload`、`skills.install`，安装通过 Inbox 给指定 Buddy 派发下载 zip 并安装的任务。
- Skills Space App 的搜索已改为 TanStack Query 按搜索词隔离缓存，输入框 debounce 后才触发请求，避免逐字请求和旧结果串到新搜索词。
...
- Space App 通信协议已收敛到 SDK，但新的独立应用契约不再通过 iframe bridge 做 command、Buddy 列表或任务投递。Space App View 调 Space App Backend；Space App Backend 调 Shadow REST 投递 Inbox task。服务端 command 仍可用 `new ShadowSpaceAppOutbox().enqueueInboxTask(task).attachTo(result)` 产生 `shadow.outbox.inboxTasks`，Shadow Server 消费后把投递结果回填到 `shadow.outbox.deliveries/errors`。
- Buddy Inbox 协议已抽到 shared/SDK：topic marker、Task Card 状态机、admission policy、Space App realtime event 常量都有统一定义；TS SDK、Python SDK、CLI 已覆盖 list/ensure/enqueue/claim/update/retry/promote。
...
- Space App command token 已支持 task binding：Buddy claim Task Card 后调用 command 可携带 `messageId/cardId/claimId`，Shadow 会校验 active claim holder，并由统一的 command-token introspection 返回 `shadow.task` 上下文。
...
- Space App command 先走 Shadow 的 command permission、approval、grant。
- Space App 返回 `shadow.outbox.inboxTasks` 后，由 Shadow 解析目标 Buddy 并投递。

Admission policy 不需要新任务模型，只是在 Inbox channel 之上增加规则：

- `allow`: 某个 Space App / Buddy / user 可直接投递。
...
- Buddy 调 Space App command 仍走 Space App grant/approval，不因 task capability 自动绕过。

## Space App Outbox

Space App 不需要调用 Buddy Inbox 专用接口。它只要在 command 结果里返回：
...
Kanban 仍是普通 Space App，不进入 Shadow 核心。它只提供通用 Kanban card graph 命令：
...
- **人类提交复杂需求**：Space App View 调 Space App Backend 获取可派发 Buddy 列表并提交 dispatch request；Space App Backend 调 Shadow REST 把需求交给协调 Buddy。
...
Skills Space App 不需要知道 Kanban；Kanban 也不需要知道 Skills。Buddy 可以在执行 Kanban Task Card 时，按权限读取 Skills Space App 的技能，并按普通 Space App command 调用 Kanban。
...
Shadow 的 Skills Space App 当前支持前端搜索、查看 package 文件、上传 zip/markdown、下载 zip，并把安装请求投递到指定 Buddy Inbox。
...
## Trainer Space App 验证

`trainer` 是对 Multica “issue 分配给 agent 后执行并回写”的一个业务 Space App 验证：
...
- 前端通过 Space App event stream 和 Query invalidation 更新提交详情。

本轮本地验证：

- 在 `http://localhost:3000/app/servers/shadow-plays/space-apps/trainer` 提交 Two Sum 到 Strategy Buddy。
- 生成提交 `sub_9pj3huu`。
...
```mermaid
flowchart TD
  A["User writes request and chooses Coordinator Buddy"] --> B["Space App View calls Space App Backend dispatch API"]
  B --> C["Space App Backend validates auth, idempotency, and calls Shadow REST"]
  C --> D["Shadow checks Inbox admission policy"]
  D --> E["Shadow publishes Task Card to Coordinator Inbox"]
  E --> F["Coordinator Buddy claims the task"]
  F --> G["Coordinator calls Kanban commands: cards.create + cards.link"]
  G --> H["Kanban stores generic cards and links"]
  H --> I["Coordinator dispatches step Task Cards to peer Buddy Inboxes"]
  I --> J["Peer Buddies execute with runtime and skills"]
  J --> K["Peer Buddies call cards.update + cards.artifacts.add"]
  K --> L["Kanban updates cards, artifacts, and audit state"]
```
...
flowchart TD
  ...
  C --> D["User or coordinator can dispatch the card through Space App Backend"]
  D --> E["Space App Backend calls Shadow REST"]
  E --> F["Shadow publishes Task Card to selected Buddy Inbox"]
  F --> G["Buddy runtime claims, runs, and updates Kanban through command tokens"]
```
...
flowchart TD
  ...
  B --> C["Skills Space App returns package metadata and files"]
  C --> D["Buddy downloads package with skills.download"]
  D --> E["Runtime writes SKILL.md plus references/scripts/assets"]
  E --> F["Buddy continues work and calls domain Space Apps as needed"]
```
...
| 评论里 `@Agent` | Space App comment command -> Inbox Task Card | Kanban `cards.comment` |
...
| skills | 独立 Skills Space App，多文件 package，可 search/upload/download/install | 已实现 MVP：支持 skills.sh 快照、`npx skills find` 动态搜索、zip 下载、Inbox 安装派发、pending approval 和 task-bound 下载 |
...
- **Task-scoped token**：Space App command token 已绑定 active claim；后续还需要把 task scopes 映射到更细的 resource action policy。
- **Skills runtime 注入**：Skills Space App 已能存储、搜索、下载完整 zip，并通过 Inbox 派发安装；当前由 Buddy 使用 task-bound `skills.download` 安装，后续再把 Claude/Codex/Copilot/OpenClaw 等 provider 的目录写入统一自动化。
- **Autopilot UI**：Shadow 不把 Autopilot 放进 Kanban；需要时应新增 automation/scheduler Space App，将 schedule/webhook/API/manual trigger 统一投递到 Inbox Task Card。
