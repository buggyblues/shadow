...
- 本方案里的“应用卡片”特指 `message.metadata.cards[]` 中 `kind: 'server_app'` 的 Server App 卡片，可打开对应 Server App 的应用内路径。
...
- 应用卡片：`message.metadata.cards[]` 中 `kind: 'server_app'` 的 Server App 卡片，必须包含 `appKey`、`title`，并通过 `action.mode = 'open_app'` 和可选 `action.path` 打开应用内路径。
...
- `unread_boost`：未读内容加权。
- `content_type_weight`：PDF/HTML/Server App 卡片略高于普通文件；图片和语音按频道活跃度中性处理。
...
- 应用卡片：打开对应 Server App 路径，无法直接打开时跳转回原消息上下文。
