...
- Kanban 只保留通用 board/card/issue 状态能力；任务投递通过 App Backend -> Shadow REST 或 Server App outbox 写入 Buddy Inbox，由协调 Buddy 负责拆分、派发和验收。
...
- Server App 通信协议已收敛到 SDK，但新的独立应用契约不再通过 iframe bridge 做 command、Buddy 列表或任务投递。App View 调 App Backend；App Backend 调 Shadow REST 投递 Inbox task。服务端 command 仍可用 `new ShadowServerAppOutbox().enqueueInboxTask(task).attachTo(result)` 产生 `shadow.outbox.inboxTasks`，Shadow Server 消费后把投递结果回填到 `shadow.outbox.deliveries/errors`。
...
- **人类提交复杂需求**：App View 调 App Backend 获取可派发 Buddy 列表并提交 dispatch request；App Backend 调 Shadow REST 把需求交给协调 Buddy。
...
```mermaid
flowchart TD
  A["User writes request and chooses Coordinator Buddy"] --> B["App View calls App Backend dispatch API"]
  B --> C["App Backend validates auth, idempotency, and calls Shadow REST"]
...
flowchart TD
  ...
  C --> D["User or coordinator can dispatch the card through App Backend"]
  D --> E["App Backend calls Shadow REST"]
