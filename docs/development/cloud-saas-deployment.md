...
2. 打开 Cloud SaaS 页面
3. 选择一个模板并开始部署
   - 如果模板要求填写 `SHADOWOB_SERVER_URL`，在本地 docker-compose 开发里优先填 pod 能访问到的地址，例如 `http://host.lima.internal:3002`
...
Cloud runner 默认设置 `OPENCLAW_SKIP_STARTUP_MODEL_PREWARM=1`，并在 entrypoint 中把
OpenRouter/LiteLLM 价格目录 fetch 的源码超时降到 `OPENCLAW_MODEL_PRICING_FETCH_TIMEOUT_MS`
配置值，避免 OpenClaw 在启动频道前长时间等待价格目录或模型预热探测。
...
- `cloud-worker` 是否在运行
- `SHADOWOB_PROVISION_URL` 是否对 `cloud-worker` 容器可达（本地 compose 通常应为 `http://server:3002`）
...
- 重新部署使用 `POST /api/cloud-saas/deployments/:id/redeploy`，会复制原部署的配置快照并创建新的 `pending` 历史记录，不再次扣费。
