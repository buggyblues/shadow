...
3. **同一可访问的 Kubernetes 凭据**
   ...
   - `server` 也需要访问集群，因为 SaaS UI 的 pods / pod logs / usage cost / token 统计是它直接查询的
...
2. 触发带 `interaction` 的命令，例如 `/office-hour`。
3. 确认 Buddy 先发送表单 / 审批组件，而不是直接纯聊天。
4. 提交表单后刷新页面，确认原表单仍显示已提交状态且按钮不可再次点击。
5. 确认后续 Buddy 回复包含命令要求的正文内容，例如问题重构、MVP 范围、路线图，再出现审批动作。
...
- Agent Pod 内是否能执行 `openclaw status --usage`
