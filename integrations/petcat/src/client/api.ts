...
export function listAssets() {
  return command<{ assets: CatAsset[] }>('cats.assets.list', {})
}

export function adoptCat(input: { name?: string; assetId?: string }) {
  return command<{ cat: PetCat }>('cats.adopt', input)
}

export function listCats() {
  return command<{ cats: CatWithAsset[] }>('cats.list', {})
}
...
export function leaderboard(input: { limit?: number }) {
  return command<{ leaderboard: CatLeaderboardEntry[] }>('cats.leaderboard', input)
}
