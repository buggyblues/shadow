...
describe('generic Kanban card data model', () => {
  ...
  it('updates status and records workspace artifacts without domain-specific output logic', () => {
  ...
  it('keeps generic card status and progress consistent when cards move across columns', () => {
    ...
    expect(reviewing?.progress).toBe(72)
  ...
  it('prevents running cards from displaying completed progress', () => {
    const card = createCard({ title: 'Prepare deliverable', createdBy: actor })
    ...
    expect(running?.progress).toBe(99)
    expect(completed?.status).toBe('done')
    expect(completed?.columnId).toBe('done')
    expect(completed?.progress).toBe(100)
  })

  it('keeps completed cards from being downgraded by stale status updates', () => {
    const card = createCard({ title: 'Prepare deliverable', createdBy: actor })
    ...
    expect(staleUpdate?.status).toBe('done')
    expect(staleUpdate?.columnId).toBe('done')
    expect(staleUpdate?.progress).toBe(100)
  ...
  it('completes cards atomically with a final summary', () => {
    ...
    expect(result?.card.progress).toBe(100)
  ...
  it('blocks completion while dependencies are unresolved', () => {
    ...
    expect(result?.card.status).not.toBe('done')
  ...
  it('normalizes persisted running cards with Workspace artifacts back to review', () => {
    ...
    expect(normalized?.buddyStatus).toBe('completed')
    expect(normalized?.progress).toBe(72)
  })
  ...
  it('reopens a generic card without emitting Inbox tasks', () => {
    ...
    expect(rerun?.card.status).toBe('queued')
  ...
  it('defers dispatch when upstream dependency cards are not done', () => {
    ...
    expect(result?.card.status).toBe('queued')
  ...
  it('blocks downstream completion while dependency cards are unresolved', () => {
    ...
    expect(blockedStatus?.status).not.toBe('done')
    expect(blockedStatus?.columnId).toBe('todo')
    expect(blockedMove?.status).not.toBe('done')
    expect(blockedMove?.columnId).toBe('todo')
    expect(blockedProgress?.status).not.toBe('done')
    expect(blockedProgress?.columnId).toBe('todo')
    expect(blockedProgress?.progress).toBeUndefined()
  ...
  it('normalizes persisted completed downstream cards with unresolved dependencies back to queued', () => {
    ...
    expect(unresolved?.status).not.toBe('done')
  ...
  it('requires dispatched artifact mime contracts before review and dependency readiness', () => {
    ...
    expect(scriptArtifact?.card.status).toBe('running')
    expect(staleReview?.status).toBe('running')
    ...
    expect(videoArtifact?.card.status).toBe('review')
    expect(videoArtifact?.card.columnId).toBe('review')
    expect(readyReview?.card.status).toBe('running')
  ...
  it('omits default artifact requirements when dispatch outputContract is explicitly null', () => {
    const card = createCard({
      ...
      prompt: 'Create linked cards and update this card with progress.',
      createdBy: actor,
    })
