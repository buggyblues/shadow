export function shellPage() {
  const viteDevServerUrl = process.env.WARBUDDY_VITE_DEV_SERVER_URL?.replace(/\/+$/, '')
  const devEntryVersion = encodeURIComponent(
    process.env.WARBUDDY_VITE_DEV_CACHE_KEY ?? String(Date.now()),
  )
