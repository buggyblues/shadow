...
export type FlashCard = Card & {
  /** Server-authoritative per-card revision, incremented on every persisted mutation. */
  revision: number
...
export interface FlashCommandEvent {
  id: string
  /** Board-local monotonic cursor. */
  seq: number
  /** Global append cursor retained for diagnostics. */
  globalSeq?: number
  boardId: string
  ...
  /** Difference between the authoritative cursor before this mutation and the client's baseCursor. */
  causalLag?: number
  createdAt: number
}
...
export interface FlashBoardEventsResult {
  events: FlashCommandEvent[]
  cursor: number
  /** Server's durable cursor at the moment this result was produced. */
  highWaterCursor?: number
  /** True when the server clipped results at limit and the client should catch up again. */
  hasMore?: boolean
}
...
const ClientMutationIdSchema = z.string().min(1).max(120).optional()
const BaseCursorSchema = z.number().int().nonnegative().optional()

const CardComposeIntentSchema = z.string().min(1).max(40)
const CardComposeKindSchema = z.string().min(1).max(40)
...
export const CardComposeDraftInputSchema = z
  .object({
    id: z.string().min(1).max(120).optional(),
    title: z.string().min(1).max(160).optional(),
    kind: CardComposeKindSchema.optional(),
...
export const CardComposePlacementSchema = z
  .object({
    x: z.number().finite().optional(),
    y: z.number().finite().optional(),
    columns: z.number().int().positive().max(12).optional(),
    gapX: z.number().finite().positive().max(1000).optional(),
    gapY: z.number().finite().positive().max(1000).optional(),
    angleJitter: z.number().finite().min(0).max(0.4).optional(),
  })
  .strict()

export const CardsComposeInputSchema = z
  .object({
    boardId: z.string().min(1).optional(),
    clientMutationId: ClientMutationIdSchema,
    baseCursor: BaseCursorSchema,
    intent: CardComposeIntentSchema.optional(),
    title: z.string().min(1).max(160).optional(),
    material: z.string().max(60000).optional(),
    instructions: z.string().max(4000).optional(),
    preferredKinds: z.array(CardComposeKindSchema).max(24).optional(),
    maxCards: z.number().int().min(1).max(40).optional(),
    placement: CardComposePlacementSchema.optional(),
    drafts: z.array(CardComposeDraftInputSchema).max(40).optional(),
  })
  .strict()
  .refine(
    (value) =>
      (typeof value.material === 'string' && value.material.trim().length > 0) ||
      (typeof value.instructions === 'string' && value.instructions.trim().length > 0) ||
      (Array.isArray(value.drafts) && value.drafts.length > 0),
    'material, instructions, or drafts are required',
  )
...
export const CardLayoutUpdateSchema = z
  .object({
    cardId: z.string().min(1),
    clientRevision: z.number().int().nonnegative().optional(),
    x: z.number().finite().optional(),
...
export const CardsLayoutUpdateInputSchema = z
  .object({
    boardId: z.string().min(1).optional(),
    clientMutationId: ClientMutationIdSchema,
    baseCursor: BaseCursorSchema,
    conflictPolicy: z.enum(['merge-layout', 'reject']).optional(),
    updates: z.array(CardLayoutUpdateSchema).min(1).max(160),
  })
  .strict()
...
export type CardComposeDraftInput = z.infer<typeof CardComposeDraftInputSchema>
export type CardsComposeInput = z.infer<typeof CardsComposeInputSchema>
...
export type CardLayoutUpdateInput = z.infer<typeof CardLayoutUpdateSchema>
export type CardsLayoutUpdateInput = z.infer<typeof CardsLayoutUpdateInputSchema>
...
export function cardMetaWithLayout(card: FlashCard): CardMeta {
  return {
    ...
    layout: card.layout,
    flash: {
      ...((card.meta as Record<string, unknown>).flash &&
      typeof (card.meta as Record<string, unknown>).flash === 'object'
        ? ((card.meta as Record<string, unknown>).flash as Record<string, unknown>)
        : {}),
