...
export function cardHash(card: Card): number {
  ...
  const linkedCount = Array.isArray(card.linkedCardIds) ? card.linkedCardIds.length : 0
  const s = `${card.title ?? ''}|${card.content?.slice(0, 80) ?? ''}|${card.kind ?? 'text'}|${card.rating ?? 0}|${card.isStreaming ? 1 : 0}|${linkedCount}|${card.autoGenerated ? 1 : 0}|${card.summary || ''}|${metaSnap}`
