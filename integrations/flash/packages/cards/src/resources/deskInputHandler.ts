...
export class DeskInputHandler {
  ...
  private handlerDoubleClick: (e: MouseEvent) => void
  ...
  private lastFlipCardId: string | null = null
  ...
  constructor(
    ...
    this.handlerDoubleClick = this.onDoubleClick.bind(this)
  ...
  private notifyViewportChange(): void {
    this.callbacks.onViewportChange?.(this.viewportSnapshot())
  }
  ...
  private attachEvents(): void {
    ...
    this.container.addEventListener('dblclick', this.handlerDoubleClick)
  ...
  private detachEvents(): void {
    ...
    this.container.removeEventListener('dblclick', this.handlerDoubleClick)
  ...
  private flipCard(cardId: string): void {
    ...
    if (this.lastFlipCardId === cardId && now - this.lastFlipTime < 180) return
    this.lastFlipCardId = cardId
  ...
  private onDoubleClick(e: MouseEvent): void {
    ...
    const { mx, my } = this.screenCoords(e)
    const s = this.fsm.getState()
    if (s.tag !== 'IDLE' || s.spaceHeld) return

    const hitCardId = this.renderer.hitTest(mx, my, this.cardsRef, this.physicsWorld.bodiesMap)
    if (!hitCardId) return
