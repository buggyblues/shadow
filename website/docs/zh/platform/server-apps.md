...
新的生产级应用默认应该走 TypeScript SDK 路径，除非有明确理由不用。目标形态是：

1. 把 `shadow-app.local.json` 当成元数据、iframe 入口、命令路径、权限、审批模式、`action`、`dataClass`、Skills 和事件名的唯一事实源。
2. 执行 `shadow-server-app typegen shadow-app.local.json src/shadow-app.generated.ts`，然后在应用服务端导入生成的 manifest。
3. 用 `defineShadowServerApp()` 和 `shadowApp.defineCommands()` 做命令分发。SDK 负责 Bearer token introspection、命令和上下文匹配、JSON Schema 校验、actor 归一化和结构化错误响应。
4. 发布 `/.well-known/shadow-app.json` 时用 `createShadowServerAppManifest()`，保证本地、Docker 和生产环境的 public/API base URL 一致重写。
5. 用 `ShadowServerAppOutbox` 表达发 Inbox task、发频道消息等 Shadow 侧副作用，不要让每个应用自造一套响应格式。
6. 嵌入式 UI 路由使用 launch runtime helper：`resolveShadowServerAppLaunchCommandContext()`、`fetchShadowServerAppLaunchInboxes()`、`deliverShadowServerAppLaunchOutbox()`。
7. 展示身份用 `shadowServerAppIdentitySnapshot()` 存储，保留 `stableKey`、`subjectKind`、`userId`、`buddyAgentId`、`ownerId`、显示名和头像 URL，让人和 Buddy 的身份分开，但渲染保持一致。Shadow 返回的头像 URL 是稳定公开身份图片，直接渲染，不要再通过私有媒体交付接口解析。
8. 轻量文件存储应用可以用 `createShadowServerAppJsonStore()` 或等价的 repository 边界。协作或实时应用要使用服务端权威状态、幂等 mutation id、事件/cursor 追赶机制，不要把 iframe 当成事实源。

安全模型里有三种身份，不能混在一起：

- **命令 actor**：由短期命令 token 代表的用户、Buddy 或 agent。使用 SDK handler context 里的 `actor` 和 `context.serverId`，不要相信请求体里的 actor 字段。
- **Iframe launch session**：在服务器 workspace 里打开应用的人。用 `shadow_launch` token 和 bridge helper 处理 launch 范围内的操作与事件订阅。
- **OAuth 绑定用户**：只有当应用需要保存用户偏好、读取 Shadow 用户资料或检查商业权益时才需要。Shadow OAuth 必须用 popup 打开，在后端换 token，并把 token 存在服务端。

第一方标准 App 应该能直接通过 Shadow launch session 打开，不要求额外配置 app-specific OAuth client。OAuth 是可选账号绑定层，不是核心登录态。

除非明确是全局资源，否则每条命令都应该以服务器为边界。实际落地就是：按 `context.serverId` 存状态，通过 manifest 声明命令权限，把用户个人偏好和服务器共享状态分开，变更后发事件让其它已打开 iframe 刷新。`kanban` 和 `qna` 是当前最成熟的参考实现；其它 integrations 不是 demo，而是真实生产项目，后续 hardening 时应该逐步收敛到同一套 SDK 形态。
...
从仓库里的成熟参考实现开始。`kanban` 是 manifest、命令协议、iframe UI、持久化和 Buddy task flow 的参考；`qna` 是内容流程、上传和持久化应用状态的参考。开发流程：
