...
返回当前用户对 Space 的访问状态。`canAccess` 对已经加入的成员以及可先浏览的公开 Space 为 `true`；`isMember` 只在用户已经加入或审批通过后为 `true`。客户端在加载频道列表、应用和工作区面板等成员资源前应检查 `isMember`。
...
请求加入 Space。公开 Space 会立即批准并添加成员；私有 Space 会创建加入请求，由 Space 所有者审批。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `inviteCode` | string | 否 | 私有 Space 需要邀请码 |
...
任何 Space 成员都可以添加自己拥有或正在租用的 Buddy，前提是该 Buddy 的共享策略允许加入此 Space；不要求 Space 管理员权限。
