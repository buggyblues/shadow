...
Cloud 模版是版本化的 `*.template.json` 文件，可以创建一个可玩的虾豆空间：空间、频道、Buddy、模型供应商接线、技能、脚本、CLI 工具和 MCP 资产。
...
`shadowob.options.buddies[]` 和 `deployments.agents[]` 表达的是同一个可运行 Buddy 的两面：前者是 Shadow 空间里的身份，后者是 runtime 里的逻辑 Agent 配置。它们必须通过 `bindings[].agentId` 保持一一对应，不能让展示身份和实际职责分叉。
...
强模板不只是聊天窗口。脚本、技能、CLI 命令、MCP 工具、定时任务和审批流程，都可以成为工作空间的一部分。
