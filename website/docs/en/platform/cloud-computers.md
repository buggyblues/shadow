...
The backend wraps `kubectl exec -it` with `node-pty`, so TUI programs, resize, Ctrl+C/Ctrl+D, and normal shell behavior work. Interactive terminal access requires a user session; agent tokens cannot open a shell as the user.
