...
function isSpaceAppCard(card: MessageCard): card is SpaceAppMessageCard {
  return (
...
function cardAppIconUrl(card: SpaceAppMessageCard) {
  const spaceApp =
    card.data?.spaceApp &&
    typeof card.data.spaceApp === 'object' &&
    !Array.isArray(card.data.spaceApp)
      ? (card.data.spaceApp as Record<string, unknown>)
      : null
  const iconUrl = stringValue(spaceApp?.iconUrl)
...
function cardPreviewUrl(card: SpaceAppMessageCard) {
  const spaceApp =
    card.data?.spaceApp &&
    typeof card.data.spaceApp === 'object' &&
    !Array.isArray(card.data.spaceApp)
  ...
  const previewUrl =
    ...
    stringValue(spaceApp?.coverUrl) ??
    stringValue(spaceApp?.coverImageUrl) ??
    stringValue(spaceApp?.previewUrl)
...
function cardAppName(card: SpaceAppMessageCard) {
  const spaceApp =
    card.data?.spaceApp &&
    typeof card.data.spaceApp === 'object' &&
    !Array.isArray(card.data.spaceApp)
      ? (card.data.spaceApp as Record<string, unknown>)
      : null
  return stringValue(spaceApp?.name) ?? card.appKey
}

export function SpaceAppCardsView({ cards }: { cards: MessageCard[] | undefined }) {
  const appCards = useMemo(() => cards?.filter(isSpaceAppCard) ?? [], [cards])
  if (appCards.length === 0) return null
  return (
    <div className="my-2 flex w-full max-w-[360px] flex-col gap-2.5">
      {appCards.map((card, index) => (
        <SpaceAppCardView key={card.id ?? `${card.appKey}:${index}`} card={card} />
      ))}
    </div>
  )
}
...
function SpaceAppCardView({ card }: { card: SpaceAppMessageCard }) {
  ...
  const content = (
    <article className="overflow-hidden rounded-xl border border-border-subtle bg-bg-primary shadow-[0_10px_28px_rgba(0,0,0,0.18)] transition hover:border-primary/25 hover:shadow-[0_14px_34px_rgba(0,0,0,0.22)]">
      ...
      <SpaceAppPreview appName={appName} iconUrl={iconUrl} previewUrl={previewUrl} />
      <div className="flex h-10 items-center gap-2 border-t border-border-subtle px-3.5">
        <SpaceAppIcon iconUrl={iconUrl} />
        <span className="min-w-0 flex-1 truncate text-xs font-bold text-text-secondary">
          {appName}
        </span>
        <span className="shrink-0 text-xs font-bold text-text-muted">{t('spaceApps.group')}</span>
      </div>
    </article>
  )
