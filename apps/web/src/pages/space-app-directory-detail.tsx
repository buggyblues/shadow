...
interface SpaceAppCatalogDetail {
  id: string
  appKey: string
  name: string
...
interface SpaceAppCatalogWithInstall extends SpaceAppCatalogDetail {
...
export function SpaceAppDirectoryDetailPage() {
  ...
  const { data: app, isLoading } = useQuery({
    queryKey: ['discover-space-app-detail', appKey, i18n.language],
    queryFn: ({ signal }) =>
      fetchApi<SpaceAppCatalogDetail>(`/api/discover/space-apps/${encodeURIComponent(appKey)}`, {
        signal,
      }),
  ...
  const { data: selectedServerCatalog = [] } = useQuery({
    queryKey: ['space-app-catalog', selectedServerKey, i18n.language],
    queryFn: ({ signal }) =>
      fetchApi<SpaceAppCatalogWithInstall[]>(
        `/api/servers/${encodeURIComponent(selectedServerKey)}/space-apps/catalog`,
        { signal },
      ),
  ...
  const installMutation = useMutation({
    mutationFn: () =>
      fetchApi(
        `/api/servers/${encodeURIComponent(selectedServerKey)}/space-apps/catalog/${app!.id}/install`,
        {
          method: 'POST',
          body: JSON.stringify({}),
        },
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['space-app-catalog', selectedServerKey] })
      queryClient.invalidateQueries({ queryKey: ['space-apps', selectedServerKey] })
      showToast(t('spaceApps.installSuccess'), 'success')
    },
    onError: (error) => {
      showToast(getApiErrorMessage(error, t, 'spaceApps.installFailed'), 'error')
    },
  })
    ...
    return (
      <div className="h-full overflow-y-auto px-4 py-5 text-text-primary">
        <GlassPanel className="mx-auto max-w-4xl p-6">
          <EmptyState
            icon={AppWindow}
            title={t('spaceApps.notFoundTitle')}
            description={t('spaceApps.notFoundDesc')}
          />
        </GlassPanel>
      </div>
    )
        ...
        <GlassPanel className="overflow-hidden p-0">
          <div className="relative min-h-[430px] bg-bg-primary md:min-h-[470px]">
            ...
            <div className="relative flex min-h-[430px] flex-col justify-between p-5 md:min-h-[470px] md:p-8">
              <button
                type="button"
                onClick={() => navigate({ to: '/discover/space-apps' })}
                ...
                <div className="flex min-w-0 items-end gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-3 max-w-4xl text-base font-bold leading-7 text-white/90 drop-shadow-[0_2px_14px_rgba(0,0,0,0.86)]">
                      {app.tagline ?? app.description ?? t('spaceApps.noTagline')}
                    </p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2">
                  ...
                  >
                    {installMutation.isPending
                      ? t('spaceApps.adding')
                      : selectedInstalled
                        ? t('spaceApps.installed')
                        : t('spaceApps.addToServer')}
                  </Button>
                </div>
              </div>
            </div>
          ...
          <div className="flex min-w-0 flex-col gap-4">
            {activeMedia ? (
              <GlassPanel className="p-5 md:p-6">
                <h2 className="mb-4 text-xl font-black text-white">{t('spaceApps.gallery')}</h2>
                <div className="relative overflow-hidden rounded-lg border border-[var(--glass-line)] bg-bg-primary/70">
                  ...
                  {gallery.length > 1 ? (
                    <>
                      <button
                        type="button"
                        aria-label={t('spaceApps.previousMedia')}
                        onClick={() => moveMedia(-1)}
                      ...
                      <button
                        type="button"
                        aria-label={t('spaceApps.nextMedia')}
                        onClick={() => moveMedia(1)}
                  ...
                  <div className="mt-4 flex items-center justify-center gap-2">
                    {gallery.map((item, index) => (
                      <button
                        key={`${item.url}:${index}`}
                        type="button"
                        aria-label={t('spaceApps.selectMedia', { index: index + 1 })}
            ...
            {overview ? (
              <GlassPanel className="p-5 md:p-6">
                <h2 className="mb-3 text-xl font-black text-white">{t('spaceApps.overview')}</h2>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-2 line-clamp-3 text-sm leading-6 text-text-secondary">
                            {command.description ?? t('spaceApps.commandNoDescription')}
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            <GlassPanel className="p-5 md:p-6">
              <h2 className="mb-4 text-xl font-black text-white">{t('spaceApps.dataAccess')}</h2>
              <div className="rounded-lg border border-[var(--glass-line)] bg-bg-secondary/42 p-4">
                <div className="flex gap-3">
                  <ShieldCheck className="mt-1 h-5 w-5 shrink-0 text-primary" />
                  <div>
                    <p className="font-bold text-white">{t('spaceApps.dataAccessTitle')}</p>
                    <p className="mt-1 text-sm leading-6 text-text-secondary">
                      {t('spaceApps.dataAccessBody')}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <aside className="flex min-w-0 flex-col gap-4">
            <GlassPanel className="p-5">
              <label
                ...
                htmlFor="space-app-server"
              >
                {t('spaceApps.chooseServer')}
              </label>
              <select
                id="space-app-server"
              ...
              <p className="mt-2 text-xs leading-5 text-text-muted">
                {selectedInstalled
                  ? t('spaceApps.installedOnSelected')
                  : selectedServerManageable
                    ? t('spaceApps.readyToAdd')
                    : t('spaceApps.adminRequired')}
              </p>
            </GlassPanel>
...
function MetadataPanel({ app }: { app: SpaceAppCatalogDetail }) {
  const { t } = useTranslation()
  const apiOrigin = originOf(app.manifest.api.baseUrl)
  return (
    <GlassPanel className="p-5">
      <div className="space-y-5">
        <MetadataBlock icon={Server} title={t('spaceApps.serverCount')}>
          <p className="font-bold text-text-secondary">
            {t('spaceApps.serverCountValue', { count: app.serverCount })}
          </p>
        </MetadataBlock>

        <MetadataBlock icon={Tags} title={t('spaceApps.categories')}>
          <ChipList values={app.categories} empty={t('spaceApps.noCategories')} />
        </MetadataBlock>

        <MetadataBlock icon={Languages} title={t('spaceApps.languages')}>
          <ChipList values={app.supportedLanguages} empty={t('spaceApps.noLanguages')} />
        </MetadataBlock>

        <MetadataBlock icon={LinkIcon} title={t('spaceApps.links')}>
          {app.links.length ? (
            ...
            <p className="text-sm text-text-muted">{t('spaceApps.noLinks')}</p>
          )}
        </MetadataBlock>

        <MetadataBlock icon={FileText} title={t('spaceApps.manifest')}>
          <div className="space-y-1 text-sm font-semibold text-text-secondary">
            <p>{t('spaceApps.versionValue', { version: app.manifest.version ?? '1' })}</p>
            {apiOrigin ? <p className="truncate">{apiOrigin}</p> : null}
          </div>
        </MetadataBlock>

        <MetadataBlock icon={LockKeyhole} title={t('spaceApps.privacy')}>
          <p className="text-sm leading-6 text-text-secondary">{t('spaceApps.privacyBody')}</p>
        </MetadataBlock>
      </div>
    </GlassPanel>
  )
