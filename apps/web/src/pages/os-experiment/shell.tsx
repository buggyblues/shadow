...
import { buddyDisplayName, OS_TOP_BAR_HEIGHT } from './utils'
  ...
  return (
    <DropdownMenu
      trigger={
        <button
          ...
          className="flex min-w-0 max-w-[260px] items-center gap-1.5 rounded-xl bg-white/10 py-0 pl-0 pr-2 text-white transition hover:bg-white/16 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70"
        >
          <ServerIcon
    ...
    >
      <DropdownMenuContent
        align="start"
        sideOffset={8}
        className="z-[520] max-h-[min(70vh,560px)] w-72 !overflow-x-hidden !overflow-y-auto overscroll-contain border-white/12 bg-bg-secondary p-2 text-text-primary shadow-[0_22px_70px_rgba(0,0,0,0.42)]"
      >
        {servers.map((entry) => {
...
export function OsBackground() {
  ...
  const shouldMove = Boolean(backgroundImage && enableBackgroundMovement && !prefersReducedMotion)
...
export function OsTopBar({
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: PointerEvent) => {
      ...
      if (target.closest('[data-os-floating-bubble-root="true"]')) return
      if (target.closest('[data-os-floating-bubble-trigger="true"]')) return
  ...
  return (
    <header className="absolute left-0 right-0 top-0 z-[600] flex h-10 items-center gap-2 border-b border-white/12 bg-black/30 px-3 text-white shadow-[0_10px_28px_rgba(0,0,0,0.18)] backdrop-blur-2xl">
      <OsAvatarMenu
        user={user}
        onExit={onExit}
          ...
          {channelTabs.slice(-6).map((tab) => {
            ...
            return (
              <div
                ...
                draggable
                className={cn(
                  'group/tab relative flex h-8 min-w-0 max-w-40 cursor-default select-none items-center gap-1.5 rounded-full border border-transparent px-3 text-left text-xs font-black transition',
                  tab.active
                    ? 'border-white/12 bg-white/14 text-white shadow-[0_8px_22px_rgba(0,0,0,0.18)]'
              ...
              >
                ...
                <button
                  ...
                  tabIndex={-1}
                  className={cn(
                    'grid h-4 w-4 shrink-0 place-items-center rounded-full text-white/50 transition hover:bg-white/14 hover:text-white',
      ...
      <div className="ml-auto flex h-10 shrink-0 items-center gap-1.5">
        {isInboxesLoading ? (
          ...
          visibleInboxes.map((entry) => {
            ...
            return (
              <button
                ...
                className="relative grid h-7 w-7 shrink-0 place-items-center overflow-visible rounded-full text-white transition hover:scale-[1.04] disabled:opacity-45"
                title={label}
              ...
              >
                {isLoading ? (
                  ...
                  <PresenceAvatar
                    ...
                    className={
                      isActive
                        ? 'ring-2 ring-primary ring-offset-2 ring-offset-black/45'
                        : undefined
                    }
                  />
                )}
          ...
          <div className="border-b border-white/10 p-3">
            <div className="flex gap-2">
              <input
                ...
                placeholder={t('channel.filterChannels')}
