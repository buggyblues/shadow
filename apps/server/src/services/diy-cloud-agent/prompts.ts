...
export function buildDiyCloudSystemPrompt(input: DiyCloudGenerateInput) {
  return [
    ...
    '',
    'Shadow CLI tooling:',
    '- The `shadowob` CLI is available to workspace buddies for programmatic interaction.',
    '- Key commands: channels (send/read messages), workspace (files), apps, oauth, marketplace, agents, shop.',
    '- Supports --json for scriptable output. Use the CLI for direct Shadow platform actions.',
    '',
    'Tool policy:',
