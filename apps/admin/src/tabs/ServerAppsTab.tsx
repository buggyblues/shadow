...
function normalizeApp(app: ServerAppIntegration): ServerAppIntegration {
  const raw = app as ServerAppIntegration & {
    categories?: unknown
...
function normalizeCatalogEntry(entry: ServerAppCatalogEntry): ServerAppCatalogEntry {
  const raw = entry as ServerAppCatalogEntry & {
    categories?: unknown
    supportedLanguages?: unknown
    gallery?: unknown
    links?: unknown
...
function metadataGaps(
  app: Pick<ServerAppIntegration, 'categories' | 'supportedLanguages' | 'coverImageUrl'>,
) {
  const gaps: string[] = []
...
export function ServerAppsTab() {
  ...
  const publishInstalledApp = async (app: ServerAppIntegration) => {
    setCatalogError('')
  ...
  const refreshInstalledApp = async (app: ServerAppIntegration) => {
    setCatalogError('')
    setRefreshingAppId(app.id)
    try {
      await apiFetch(`/server-apps/${app.id}/refresh`, { method: 'POST' })
      await load()
    } catch (error) {
      setCatalogError(error instanceof Error ? error.message : String(error))
    } finally {
  ...
  const refreshCatalogEntry = async (entry: ServerAppCatalogEntry) => {
    setCatalogError('')
    setRefreshingCatalogId(entry.id)
    try {
      await apiFetch(`/server-app-catalog/${entry.id}/refresh`, { method: 'POST' })
      await load()
    } catch (error) {
      setCatalogError(error instanceof Error ? error.message : String(error))
    } finally {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 rounded-2xl border border-zinc-800 bg-zinc-950/70 p-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 className="text-xl font-black text-white">应用管理</h2>
          <p className="mt-1 text-sm text-zinc-400">
            已安装应用进入待收录队列，上架后会出现在发现页。
          </p>
        </div>
            ...
            <div className="grid gap-3 md:grid-cols-2 2xl:grid-cols-3">
              {unlistedApps.map((app) => (
                <PublishQueueCard
                  ...
                  onPublish={() => publishInstalledApp(app)}
                />
              ))}
            </div>
          ) : (
            <EmptyPanel text={query ? '没有匹配的待收录应用。' : '所有已安装应用都已收录。'} />
          )}
        </section>
        ...
        <section className="rounded-xl border border-zinc-800 bg-zinc-900 p-4">
          ...
          <div className="space-y-3">
            <input
              ...
              placeholder="https://example.com/shadow-app.json"
            ...
            {apps.map((app) => (
              <tr key={app.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
                ...
                <td className="px-4 py-3 align-top">
                  <div className="flex flex-col items-start gap-2">
                    <button
                      onClick={() => refreshInstalledApp(app)}
                    ...
                    {!app.inCatalog && (
                      <button
                        onClick={() => publishInstalledApp(app)}
...
function MetadataChips({
  categories,
  supportedLanguages,
}: Pick<ServerAppIntegration, 'categories' | 'supportedLanguages'>) {
  return (
    ...
    <article className="overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950">
      ...
      <div className="p-3">
        ...
        <div className="mt-3 flex items-center justify-between gap-3 text-xs text-zinc-500">
          <span>
            {entry.commandCount} 命令 · {entry.serverCount} 服务器
          </span>
