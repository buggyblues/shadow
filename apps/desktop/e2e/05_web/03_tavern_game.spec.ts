...
const TAVERN_NPCS = [
  { name: '酒保 · Barkeep', description: 'The innkeeper who runs the tavern, serving drinks and gossip.' },
...
const TAVERN_CHANNELS = [
  { name: '大厅', type: 'text', topic: 'The main hall of the tavern — all adventurers gather here.' },
  { name: '酒吧', type: 'text', topic: 'The bar counter — order drinks and chat with the barkeep.' },
  ...
  { name: '公告板', type: 'announcement', topic: 'Quest board — check available quests and bounties.' },
]
...
test.describe.serial('Tavern Game — OAuth Platform App E2E', () => {
  test('full tavern game scenario: authorize → create server → NPCs → channels → messages', async ({
    ...
    try {
      ...
      // Intercept callback redirect to capture the auth code
      let capturedCode = ''
