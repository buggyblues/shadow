...
const SHADOWOB_APP_COMMANDS_SOURCE = 'shadow://skills/shadow-server-app'
...
export const shadowAppSlashCommands: RuntimeSlashCommand[] = [
  {
    ...
    body: [
      ...
      'Create or keep the App source under `$SHADOWOB_WORKSPACE`, `/workspace`, or the Cloud runner home directory so publish metadata and backup tracking can accept the paths.',
      ...
      'Publish with `shadowob app publish --port <port> --manifest-file shadow-app.local.json --source-path "$PWD" --json`; the Cloud runtime should auto-detect deployment, agent, and the target server from the current Inbox/channel context.',
  ...
  },
  {
    ...
    body: [
      ...
      'Keep the App source under `$SHADOWOB_WORKSPACE`, `/workspace`, or the Cloud runner home directory so publish metadata and backup tracking can accept the paths.',
      ...
      'Republish with `shadowob app publish --port <port> --manifest-file shadow-app.local.json --source-path "$PWD" --json`; the Cloud runtime should auto-detect deployment, agent, and the target server from the current Inbox/channel context.',
